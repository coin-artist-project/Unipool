...
contract('Unipool', function ([_, wallet1, wallet2, wallet3, wallet4]) {
    describe('Unipool', async function () {
        beforeEach(async function () {
            ...
            // For multiple tokens
            this.snx2 = await Snx.new();
            this.snx3 = await Snx.new();
            await this.snx2.mint(this.pool.address, web3.utils.toWei('1000000'));
            await this.snx3.mint(this.pool.address, web3.utils.toWei('1000000'));
        ...
        it('Two stakers with the same stakes wait 1 w', async function () {
            ...
            expect(await this.pool.countStakers()).to.be.bignumber.equal('0');

            await this.pool.stake(web3.utils.toWei('1'), { from: wallet1 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('1');
            expect((await this.pool.stakers('0'))).to.be.equal(wallet1);
            await this.pool.stake(web3.utils.toWei('1'), { from: wallet2 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('2');
            expect((await this.pool.stakers('1'))).to.be.equal(wallet2);
        ...
        it('Three stakers with the different (1:3:5) stakes wait 3 weeks', async function () {
            ...
            expect(await this.pool.countStakers()).to.be.bignumber.equal('3');
            expect((await this.pool.stakers('2'))).to.be.equal(wallet3);
            ...
            // Stakers check -- have all three wallets stake more, then check that the number of stakers remains the same
            await this.pool.stake(web3.utils.toWei('5'), { from: wallet1 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('3');
            await this.pool.stake(web3.utils.toWei('5'), { from: wallet2 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('3');
            await this.pool.stake(web3.utils.toWei('5'), { from: wallet3 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('3');

            // Pay to give rewards to the wallets
            await this.pool.getReward(wallet1);
            await this.pool.getReward(wallet2);
            await this.pool.getReward(wallet3);

            await timeIncreaseTo(this.started.add(time.duration.weeks(4)));

            expect(await this.pool.earned(wallet1)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('0'));
            expect(await this.pool.earned(wallet2)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('0'));
            expect(await this.pool.earned(wallet3)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('0'));
        ...
        it('Multiple tokens: Two stakers with the same stakes wait 1 w', async function () {
            // 72000 SNX per week for 3 weeks
            await this.pool.notifyRewardAmount(web3.utils.toWei('72000'), { from: wallet1 });
            ...
            await timeIncreaseTo(this.started.add(time.duration.weeks(10)));

            // Try to switch reward token, fail
            try { await this.pool.setNewRewardToken(this.snx2.address); assert(false); }
            catch(ex) { assert(String(ex).indexOf('Must clear previous token') !== -1); }

            // Disallow non-owner to empty the contract
            try { await this.pool.retrieveRemainingRewardTokens({ from: wallet1 }); assert(false); }
            catch(ex) { assert(String(ex).indexOf('caller is not the owner') !== -1); }

            // Pay to give rewards to the wallets
            await this.pool.getReward(wallet1);
            await this.pool.getReward(wallet2);
            await this.pool.getReward(wallet3); // Not staking

            // Empty the contract
            await this.pool.retrieveRemainingRewardTokens();

            // Set the new contract
            await this.pool.setNewRewardToken(this.snx2.address);
            ...
            await timeIncreaseTo(this.started.add(time.duration.weeks(11)));
            ...
            // Set the new contract
            await this.pool.setNewRewardToken(this.snx3.address);
            ...
            await this.pool.stake(web3.utils.toWei('1'), { from: wallet3 });
            expect(await this.pool.countStakers()).to.be.bignumber.equal('3');
            expect((await this.pool.stakers('2'))).to.be.equal(wallet3);

            await timeIncreaseTo(this.started.add(time.duration.weeks(12)));

            expect(await this.pool.rewardPerToken()).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('108000'));
            expect(await this.pool.earned(wallet1)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('36000'));
            expect(await this.pool.earned(wallet2)).to.be.bignumber.almostEqualDiv1e18('0');
            expect(await this.pool.earned(wallet3)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('36000'));
